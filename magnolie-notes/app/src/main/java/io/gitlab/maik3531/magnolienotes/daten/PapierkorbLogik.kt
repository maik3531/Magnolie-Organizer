...
object PapierkorbLogik {
    ...
    fun loescheAufgabe(input: Bestand, id: String, now: Long = System.currentTimeMillis()): Bestand {
        ...
        val normal = AufgabenHierarchie.normalisieren(input.aufgaben)
        val normalizedValue = normal.first { it.id == id }
        return input.copy(aufgaben = AufgabenHierarchie.loeschenUndKinderHochheben(normal, normalizedValue.uid),
            papierkorb = add(input, neu("task", normalizedValue.anzeigeTitel,
                aufgabe = normalizedValue.copy(), now = now)))
    }
    ...
    fun wiederherstellen(input: Bestand, trashId: String, now: Long = System.currentTimeMillis()): Bestand? {
        ...
        result = when (item.art) {
            ...
            "task" -> {
                val task = item.aufgabe!!
                val restored = if (task.elternUid.isNotEmpty() && result.aufgaben.none { it.uid == task.elternUid })
                    task.copy(elternUid = "", reihenfolge = result.aufgaben.count { it.elternUid.isEmpty() })
                else task
                result.copy(aufgaben = AufgabenHierarchie.normalisieren(result.aufgaben + restored))
            }
