...
class PortableArchivTest {
    ...
    @Test fun `Archiv Version 1 wird gelesen und hierarchisch migriert`() {
        val legacy = Bestand(aufgaben = listOf(Aufgabe("legacy", "Alt")))
        val archiv = PortableArchiv.erstellen(legacy, passwort, 1)
        assertEquals(1, archiv[16].toInt())
        val restored = Ablage.json.decodeFromString<Bestand>(
            PortableArchiv.pruefen(archiv, passwort).bestandJson)
        assertEquals("legacy", restored.aufgaben.single().id)
        assertTrue(restored.aufgaben.single().uid.isNotEmpty())
    }
    ...
    @Test fun `Header Manipulation Trunkierung und Parametergrenzen werden abgelehnt`() {
        ...
        listOf(16 to 3, 17 to 2, 18 to 2, 19 to 15, 24 to 13).forEach { (offset, value) ->
