...
internal sealed partial class BridgeDispatcher : IDisposable
{
    ...
    internal BridgeDispatcher(MainForm form, WindowsPaths paths)
    {
        ...
        http.DefaultRequestHeaders.UserAgent.ParseAdd("Magnolie-Organizer-Windows/2.0.6");
    }
    ...
    private async Task ExportGesamtarchivAsync(string password)
    {
        ...
        try
        {
            ...
            var version = typeof(BridgeDispatcher).Assembly.GetName().Version?.ToString(3) ?? "2.0.6";
    ...
    private string AppVersion => typeof(BridgeDispatcher).Assembly.GetName().Version?.ToString(3) ?? "2.0.6";
