...
internal static class RestoreSelection
{
        ...
        IReadOnlyCollection<string> areas, string mode)
    {
        ...
        var result = areas.Contains("all") ? snapshot : current.DeepClone().AsObject();
    ...
    private static void PreserveAttachments(JsonNode restored, JsonNode current)
    {
        ...
        void WalkCurrent(JsonNode node)
        {
            ...
            {
                var key = value["sha256"]?.ToString() ?? value["attachment_id"]?.ToString() ??
                ...
                foreach (var name in new[] { "daten", "data" })
                    if (key.Length > 0 && value[name]?.ToString() is string text &&
                        text.StartsWith("data:", StringComparison.OrdinalIgnoreCase)) data[name + "\0" + key] = text;
        ...
        void WalkRestored(JsonNode node)
        {
            if (node is JsonObject value)
            {
                var key = value["sha256"]?.ToString() ?? value["attachment_id"]?.ToString() ??
                    value["id"]?.ToString() ?? value["name"]?.ToString() ?? "";
                foreach (var name in new[] { "daten", "data" })
                    if (key.Length > 0 && value[name]?.ToString() == "" && data.TryGetValue(name + "\0" + key, out var text)) value[name] = text;
