...
const locales = ["en", ...fs.readdirSync(path.join(root, "magnolie-organizer-2.0.0/po"))
...
for (const web of [path.join(root, "magnolie-organizer-2.0.0/web"), path.join(root, "Magnolie-Organizer-Windows-2.0.0/app/web")]) {
  for (const language of locales) {
    ...
    try {
      ...
      const catalog = language === "en" ? null : JSON.parse(fs.readFileSync(path.join(root,
        "Magnolie-Organizer-Windows-2.0.0/app/native-i18n.json"))).locales[language];
      if (catalog) {
