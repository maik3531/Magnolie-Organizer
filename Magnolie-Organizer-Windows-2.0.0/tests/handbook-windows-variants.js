...
vm.runInContext(platformSource.replace("const variants =", "const variants = window.__windowsVariants ="), context,
...
for (const locale of locales) {
  ...
  assert.ok(!new RegExp(`Magnolie-Organizer-Windows-(?!${version.replaceAll(".", "\\.")})\\d+\\.\\d+\\.\\d+-Setup-x64\\.exe`).test(installing),
...
for (const [sourceLocale, [, expectedContentHash]] of Object.entries(importedLocales)) {
  ...
  const stableContent = JSON.stringify(localeObject(variants, locale))
    .replaceAll(installerName, "Magnolie-Organizer-Windows-2.0.0-Setup-x64.exe");
  assert.strictEqual(hash(stableContent), expectedContentHash,
    `Importinhalt weicht für ${sourceLocale} ab`);
}
for (const [locale, expectedHash] of Object.entries(preservedLocaleHashes)) {
  const stableContent = JSON.stringify(localeObject(variants, locale))
    .replaceAll(installerName, "Magnolie-Organizer-Windows-2.0.0-Setup-x64.exe");
  assert.strictEqual(hash(stableContent), expectedHash,
...
(async () => {
  ...
  for (const [index, locale] of locales.entries()) {
    ...
    for (const pageId of pageIds.filter((id) => id !== "appearance-sound-and-anniversaries")) {
      ...
      assert.ok(page, `Windows-Variantenseite fehlt für ${pageId}/${locale}`);
      assert.strictEqual(page.inhalt, variants[pageId][locale],
        `DOM-Variante wurde für ${pageId}/${locale} nicht angewendet`);
    }
    assert.ok(!pages.some((page) => page.id === "appearance-sound-and-anniversaries"),
      `Ersetzte Reminder-Seite ist für ${locale} zurückgekehrt`);
