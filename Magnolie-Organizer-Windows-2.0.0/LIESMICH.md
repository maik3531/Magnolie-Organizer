...
`Magnolie-Organizer-Windows-2.0.5-x64.zip`.
...
Alternativ installiert `Magnolie-Organizer-Windows-2.0.5-Setup-x64.exe` die
...
`build\BuildSource.ps1` als `Magnolie-Organizer-Windows-2.0.5-Source.zip`.
...
`Magnolie-Organizer-Windows-2.0.5-Setup-x64.exe` samt entsprechendem
...
Metadaten tragen immer den Namen `Magnolie-Organizer-Windows-2.0.5-Setup-x64.exe`.
...
```powershell
    .\build\UpdateManifest.ps1 -Installer .\Magnolie-Organizer-Windows-2.0.5-Setup-x64.exe -ExpectedPublisher 'CN=Exakter Zertifikatinhaber' -UpdateManualWindows
```
