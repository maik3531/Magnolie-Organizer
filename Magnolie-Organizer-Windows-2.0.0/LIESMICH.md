...
`Magnolie-Organizer-Windows-2.0.0-x64.zip`.
...
Alternativ installiert `Magnolie-Organizer-Windows-2.0.0-Setup-x64.exe` die
...
`build\BuildSource.ps1` als `Magnolie-Organizer-Windows-2.0.0-Source.zip`.
...
`Magnolie-Organizer-Windows-2.0.0-Setup-x64.exe` samt entsprechendem
...
Metadaten tragen immer den Namen `Magnolie-Organizer-Windows-2.0.0-Setup-x64.exe`.
...
Nach Veröffentlichung aktualisiert folgender manuelle Windows-Befehl die Windows-
...
```powershell
.\build\UpdateManifest.ps1 -Installer .\Magnolie-Organizer-Windows-2.0.0-Setup-x64.exe -ExpectedPublisher 'CN=Exakter Zertifikatinhaber'
```
