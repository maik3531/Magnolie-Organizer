...
const reviewPfad = [path.resolve(WEB, "..", "REVIEW-ENTSCHEIDUNGEN.md"),
  path.resolve(WEB, "..", "..", "Entwicklungsnotizen", "REVIEW-ENTSCHEIDUNGEN.md")]
  .find((datei) => fs.existsSync(datei));
const reviewEntscheidungen = fs.readFileSync(
  reviewPfad, "utf8");
...
(async () => {
  ...
  assert.ok(liesmich.includes("Strg+Alt+H") &&
    reviewEntscheidungen.includes("WebKitGTK 2.38") &&
    reviewEntscheidungen.includes("voreingestellt deaktiviert"),
  ...
  assert.strictEqual(contributorDokument.querySelector("#contributor-wasserzeichen")
    .textContent, "No valid subscription",
  "im offiziellen Build fehlt das Contributor-Wasserzeichen");
  ...
  ["every day", "every week on the same weekday", "every month on the same day",
    "every year on the same date"], "englische Wiederholungsregeln fehlen");
  ...
  assert.strictEqual(enSD.querySelector("#einstellungen-inhalt h3").textContent,
    "Synchronization (online accounts)", "englische Synchronisationsseite fehlt");
  ...
  assert.ok(datenSave && !JSON.parse(datenSave.text).letzteSyncs.adressbuecher["buch-privat"],
  ...
  assert.ok(baselineSave !== datenSave &&
    JSON.parse(baselineSave.text).letzteSyncs.adressbuecher["buch-privat"].initialisiert,
  ...
  assert.ok(!enST.daten().letzteSyncs.adressbuecher["buch-privat"],
    "ein Baseline-Speicherfehler lässt die Quelle im Speicher initialisiert");
  ...
  assert.ok(enUeber.querySelector(".ueber-fassung").textContent.includes("Version 2.0.0") &&
  ...
  const enUpdateUrl = "https://gitlab.com/maik3531/mint-forgs/-/raw/main/" +
    "Magnolie-Organitzer/magnolie-organizer_2.0.1_all.deb";
  enSW.App.updateErgebnis({ ok: true, aktuell: false, version: "2.0.1",
  ...
  assert.ok(enSD.querySelector("#update-stand").textContent.includes(
    ...
    enSD.querySelector("#update-herunterladen").textContent.includes("2.0.1") &&
  ...
  assert.strictEqual(enSD.querySelectorAll("#gesundheit-datum-vorschlaege option").length, 3,
    "Gesundheitsdaten bieten nicht heute, gestern und vorgestern an");
  ...
  "ein gespeicherter Vitalwert erzeugt trotz abgeschalteter Planung ein Kalendersymbol");
  Object.assign(enST.daten().einstellungen.kalender.gesundheitPlanung.vital,
    { an: true, von: enST.isoHeute(), zeit: "08:00", intervallTage: 1 });
  ...
  assert.strictEqual($("#sicherung-bestaetigen").textContent, "Vollständig ersetzen",
  ...
  T.oeffneTerminBlatt(null, T.zustand().kalender.tag);
  assert.strictEqual($("#tb-datum-von").type, "date",
    "aktivierte Kalenderauswahl erzeugt kein natives Datumsfeld");
  ...
  assert.ok($(".ueber-fassung").textContent.includes("2.0.0"),
  ...
  assert.ok(T.istNeuereFassung("2.0.1") && !T.istNeuereFassung("2.0.0"),
  ...
  w.App.updateErgebnis({ ok: true, aktuell: false, version: "2.0.1",
    url: "https://gitlab.com/maik3531/mint-forgs/-/raw/main/" +
      "Magnolie-Organitzer/magnolie-organizer_2.0.1_all.deb" });
  assert.ok($("#update-stand").textContent.includes("2.0.1"),
    "gefundene Fassung erscheint nicht unter Über");
  assert.ok($("#update-herunterladen"), "Downloadknopf für neue Fassung fehlt");
  assert.strictEqual(T.daten().einstellungen.update.letzteVersion, "2.0.1",
  ...
  hw.App.updateErgebnis({ ok: true, aktuell: true, version: "2.0.0", url: "",
  ...
  const smsTelefonStand = { peers: [{ device_id: "telefon-1", display_name: "Mobiltelefon",
    ...
    grants: { grants: { sms_send: true } } }] };
  const zeichneKontaktAktionen = (telefone, sozialeMedien = []) => {
  ...
  assert.ok(kontaktAktionenNachrichten.some((nachricht) =>
    nachricht.cmd === "sozial" && nachricht.dienst === "phone" &&
    nachricht.wert === "0203 111111"),
  "eine einzelne Rufnummer verwendet nicht den sicheren phone-Systemweg");
  ...
  kontaktD.querySelector(".sms-dialog .dialog-knoepfe button:last-child").click();
  ...
  assert.ok(kontaktAktionenNachrichten.some((nachricht) =>
    nachricht.cmd === "sozial" && nachricht.dienst === "phone" &&
    nachricht.wert === "0173 888888"),
  "die ausgewählte Nummer wird nicht über den sicheren phone-Systemweg angerufen");
  ...
  geraetW.App.geraetOeffnen({ kennung: "partner-intern-1", name: "Annas Telefon",
    status: {} });
  assert.strictEqual(geraetD.querySelector("#geraet-titel").textContent, "Annas Telefon");
