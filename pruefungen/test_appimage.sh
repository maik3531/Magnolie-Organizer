...
for befehl in g-ir-inspect ldd nm; do
...
if command -v timeout >/dev/null && command -v xvfb-run >/dev/null; then
    appimage_gui_start() {
        if command -v dbus-run-session >/dev/null; then
            dbus-run-session -- timeout 5s xvfb-run -a "$APPDIR/AppRun"
        else
