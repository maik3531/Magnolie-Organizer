...
komplexe_regel = m.ics_lesen(
    "BEGIN:VEVENT\r\nSUMMARY:Vierzehntäglich\r\n"
    "DTSTART:20260805T190000\r\nRRULE:FREQ=WEEKLY;INTERVAL=2\r\n"
    "END:VEVENT\r\n")
pruefe(komplexe_regel["termine"][0]["wiederholung"]["art"] == "none" and
       komplexe_regel["wiederholend"] == 1,
       "nicht abbildbare RRULEs werden weiterhin ehrlich gemeldet")
...
mit_zusatz = m.ics_lesen(
    ...
    "END:VEVENT\r\n")
pruefe(mit_zusatz["termine"][0]["wiederholung"]["art"] == "none" and
       mit_zusatz["wiederholend"] == 1,
       "RDATE ohne RRULE wird als bewusste Produktgrenze gemeldet")
...
pruefe(mehrtaegige_serie["termine"][0]["icsKomplex"] and
       mehrtaegige_serie["termine"][0]["wiederholung"]["art"] == "none",
       "mehrtägige Serien werden ohne verschobenes Enddatum nicht angenähert")
...
sozial = m.sozial_oeffnen(
    "sms", "+49 170 1234567", sozial_aufrufe.append,
    lambda name: "/usr/bin/xdg-open" if name == "xdg-open" else None,
    valent_aktion=lambda _nummer: False)
pruefe(sozial["ok"] and sozial_aufrufe[0] ==
       ["/usr/bin/xdg-open", "sms:+491701234567"],
       "SMS wird als adressierter Entwurf an Valent oder KDE Connect übergeben")
sozial_aufrufe = []
sozial = m.sozial_oeffnen(
    "sms", "+49 170 1234567", sozial_aufrufe.append, lambda _name: None,
    valent_aktion=lambda nummer: nummer == "+491701234567")
pruefe(sozial["ok"] and sozial["womit"] == "valent" and not sozial_aufrufe,
       "Valent-Mint erhält den Empfänger direkt über seine Compose-Aktion")
sozial_aufrufe = []
sozial = m.sozial_oeffnen(
...
pruefe(not geaendert_eds and
       any(t["uid"] == "extern-komplex" and
           t["titel"] == "Lokale alte Kopie" for t in kal_termine),
...
pruefe(probe.nutzlast["bericht"].startswith("Termine:") and
...
pruefe(m.PROGRAMM_FASSUNG == "2.0.0", "Programmkern trägt die neue Fassung")
...
update_summe = "ab" * 32
update_paket = m.UPDATE_BASIS + "magnolie-organizer_2.0.1_all.deb"
update_signatur = m.base64.b64encode(update_privat.sign(
    m.update_signatur_nachricht("2.0.1", update_paket, update_summe))).decode("ascii")
update_xml = ("<?xml version='1.0'?><update><version>2.0.1</version>"
...
pruefe(version == "2.0.1" and paket.endswith("_2.0.1_all.deb"),
...
aktuell_paket = m.UPDATE_BASIS + "magnolie-organizer_2.0.0_all.deb"
aktuell_signatur = m.base64.b64encode(update_privat.sign(
    m.update_signatur_nachricht("2.0.0", aktuell_paket, update_summe))).decode("ascii")
aktuell_xml = ("<update><version>2.0.0</version><deb>" + aktuell_paket +
...
pruefe(update_aktuell["ok"] and update_aktuell["aktuell"],
...
handbuch_xml = aktuell_xml.replace("</update>",
    "<manual><version>1.9.8</version><linux><deb>" + handbuch_paket +
    "</deb><sha256>" + handbuch_summe + "</sha256></linux></manual></update>")
handbuch_update = m.update_pruefen(lambda _url: handbuch_xml)
pruefe(handbuch_update["ok"] and handbuch_update["aktuell"] and
       handbuch_update["version"] == "2.0.0" and
...
ungueltiges_handbuch = m.update_manifest_pruefen(handbuch_xml.replace(
    handbuch_paket, m.UPDATE_BASIS + "magnolie-organizer_1.9.8_all.deb"))
...
altes_update_xml = ("<update><version>2.0.1</version><deb>" +
                    update_paket + "</deb></update>")
update_ohne_schluessel = m.update_pruefen(lambda _url: altes_update_xml)
pruefe(update_ohne_schluessel["ok"] and
       not update_ohne_schluessel["aktuell"] and
       update_ohne_schluessel["sha256"] == "",
       "ohne Release-Schlüssel arbeitet die Aktualisierung wie bisher")
update_veraendert = m.update_pruefen(
    lambda _url: update_xml.replace("2.0.1", "2.0.2"), update_oeffentlich)
...
pruefe(hilfe_code_en == 0 and "Usage:" in hilfe_en and
       ...
       "magnolie-organizer --tray-start" in hilfe_en and
       hilfe_code_de == 0 and "Aufruf:" in hilfe_de and
       "Programm öffnen" in hilfe_de and
       "magnolie-organizer --tray-start" in hilfe_de,
...
pruefe("8737/tcp" in ufw_profil and "8737/udp" in ufw_profil and
       "5353/udp" in ufw_profil,
       "das UFW-Profil erlaubt Dual-Stack-Transport, Alt-Rundruf und mDNS")
...
pruefe("kontakte" not in auswahl and "notizen" not in auswahl,
       "Adressen und Notizen geraten nie in den Erinnerungsbestand")
