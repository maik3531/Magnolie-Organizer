...
pruefe(m.PROGRAMM_FASSUNG == "2.0.14", "Programmkern trägt die neue Fassung")
...
update_summe = "ab" * 32
update_paket = m.UPDATE_BASIS + "magnolie-organizer_2.0.15_all.deb"
update_signatur = m.base64.b64encode(update_privat.sign(
    m.update_signatur_nachricht("2.0.15", update_paket, update_summe))).decode("ascii")
update_xml = ("<?xml version='1.0'?><update><version>2.0.15</version>"
...
pruefe(version == "2.0.15" and paket.endswith("_2.0.15_all.deb"),
...
pruefe(update_neu["ok"] and not update_neu["aktuell"] and
       update_neu["version"] == "2.0.15" and
       update_neu["sha256"] == update_summe and update_neu["url"] == update_paket,
       "eine Debian-Installation erhält das signierte Debian-Paket")
appimage_paket = m.UPDATE_BASIS + "Magnolie-Organizer-2.0.15-x86_64.AppImage"
appimage_summe = "ef" * 32
appimage_xml = ("<update><version>2.0.15</version><deb>" + update_paket +
...
appimage_signatur = m.base64.b64encode(update_privat.sign(
    m.update_signatur_nachricht("2.0.15", update_paket, update_summe,
...
aarch64_paket = m.UPDATE_BASIS + "Magnolie-Organizer-2.0.15-aarch64.AppImage"
aarch64_xml = ("<update><version>2.0.15</version><deb>" + update_paket +
               "</deb><sha256>" + update_summe + "</sha256><appimage>"
...
aktuell_paket = m.UPDATE_BASIS + "magnolie-organizer_2.0.14_all.deb"
aktuell_signatur = m.base64.b64encode(update_privat.sign(
    m.update_signatur_nachricht("2.0.14", aktuell_paket, update_summe))).decode("ascii")
aktuell_xml = ("<update><version>2.0.14</version><deb>" + aktuell_paket +
...
pruefe(update_aktuell["ok"] and update_aktuell["aktuell"] and
       update_aktuell["version"] == "2.0.14" and not update_aktuell["url"],
...
handbuch_signatur = m.base64.b64encode(update_privat.sign(
    m.update_signatur_nachricht("2.0.14", aktuell_paket, update_summe,
                               manual_version="1.9.8", manual_linux=handbuch_paket,
                               manual_linux_sha=handbuch_summe))).decode("ascii")
handbuch_xml = ("<update><version>2.0.14</version><deb>" + aktuell_paket +
...
pruefe(handbuch_update["ok"] and handbuch_update["aktuell"] and
       handbuch_update["version"] == "2.0.14" and
...
altes_update_xml = ("<update><version>2.0.14</version><deb>" +
...
update_veraendert = m.update_pruefen(
    lambda _url: update_xml.replace("<version>2.0.15</version>",
