...
def check_cross_language(fixtures):
    ...
    project = ROOT / 'Magnolie-Organizer-Windows-2.0.0/tests/CoreTests.csproj'
    ...
    web = subprocess.run(command + [str(ROOT / 'Magnolie-Organizer-Windows-2.0.0/tests/recurrence-integration.js')], input=json.dumps(payload), capture_output=True, text=True)
