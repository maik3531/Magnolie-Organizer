    ...
    for (const key of ["kapitel", "titel", "inhalt"]) {
      ...
      if (key === "inhalt") {
        ...
        if (page.titel === "Sharing entries securely") {
          ...
          assert.ok(data.messages[page[key]].includes("2.0.12") &&
...
function checkLocale(locale, expected) {
  ...
  for (const preserved of ["backing-up-and-restoring", "magnolie-organizer_2.0.12_all.deb",
    "sudo apt install ./magnolie-organizer_2.0.12_all.deb", "wttr.in",
    "maik3531@gmail.com", "2.0.12"]) {
...
try {
  ...
  assert.ok(englishText.includes("sudo dnf install ./magnolie-organizer-2.0.12-1.noarch.rpm"));
  assert.ok(englishText.includes("rpmbuild --rebuild magnolie-organizer-2.0.12-1.src.rpm"));
  assert.ok(englishText.includes("sudo dnf upgrade ./magnolie-organizer-2.0.12-1.noarch.rpm"));
  ...
  assert.ok(python.includes('PROGRAMM_FASSUNG = "2.0.12"') && python.includes('"--version"'));
  ...
  assert.ok(changelog.startsWith("magnolie-handbuch (2.0.12)"));
  assert.ok(pot.includes('"Project-Id-Version: Magnolie Handbook 2.0.12'));
