...
def test_twenty_manual_locales_match_both_desktops_and_android():
    ...
    parser = load(ROOT / "magnolie-organizer-2.0.0/werkzeuge/desktop_pot_merge.py", "identifier_catalog")
    ...
    for locale, values in data.TEXT.items():
        ...
        resources = ET.parse(ROOT / "magnolie-notes-1.0.13/app/src/main/res" / folder / "strings.xml")
        ...
        if locale == "en":
            continue
        for directory in ("magnolie-organizer-2.0.0/po", "Magnolie-Organizer-Windows-2.0.0/app/po"):
