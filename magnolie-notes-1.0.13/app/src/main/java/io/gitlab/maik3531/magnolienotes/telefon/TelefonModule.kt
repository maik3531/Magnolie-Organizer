...
object TelefonModulStatus {
    ...
    fun notifications(context: Context, storage: TelefonAblage = TelefonAblage.get(context)): Boolean =
        notifications(storage.selectedPackages(), notificationAccess(context))

    fun notifications(selectedPackages: Set<String>, access: Boolean): Boolean =
        selectedPackages.isNotEmpty() && access
