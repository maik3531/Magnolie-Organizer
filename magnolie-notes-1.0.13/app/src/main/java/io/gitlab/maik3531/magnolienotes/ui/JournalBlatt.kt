    ...
    Column(Modifier.fillMaxSize().background(Magnolie.papier).verticalScroll(rememberScrollState())) {
        ...
        Abschnitt(ueberschrift = stringResource(R.string.journal_titel),
            ...
            Schreibfeld(maximumText, stringResource(R.string.journal_maximum),
                beiAenderung = { eingabe ->
                    maximumText = eingabe.filter(Char::isDigit).take(3)
                    maximumText.toIntOrNull()?.takeIf { it in 1..100 }?.let(handlungen.beiMaximum)
                })
        ...
        Abschnitt(ueberschrift = stringResource(R.string.auto_backup_titel),
            ...
            Schreibfeld(autoAufbewahrung, stringResource(R.string.auto_backup_aufbewahrung),
                beiAenderung = { eingabe ->
                    autoAufbewahrung = eingabe.filter(Char::isDigit).take(2)
                    autoAufbewahrung.toIntOrNull()?.takeIf { it in 2..30 }
