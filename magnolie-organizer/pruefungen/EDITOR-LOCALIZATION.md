...
```sh
python3 magnolie-organizer/werkzeuge/desktop_pot_merge.py --report-dir /tmp/opencode/editor-localization-merge
...
```sh
sh magnolie-organizer/pruefungen/editor_localization_isolated.sh --web linux --menus-only
sh magnolie-organizer/pruefungen/editor_localization_isolated.sh --web windows --menus-only
sh magnolie-organizer/pruefungen/editor_localization_isolated.sh --web linux --history-only
sh magnolie-organizer/pruefungen/editor_localization_isolated.sh --web windows --history-only
MAGNOLIE_CONTEXT_EVIDENCE=/tmp/opencode/editor-asr05-regression sh magnolie-organizer/pruefungen/context_edit_isolated.sh --web linux
MAGNOLIE_CONTEXT_EVIDENCE=/tmp/opencode/editor-asr05-regression sh magnolie-organizer/pruefungen/context_edit_isolated.sh --web windows
systemd-run --user --scope -p MemoryMax=6G -p CPUQuota=200% timeout --kill-after=5s 240s python3 magnolie-organizer-windows/tests/editor_localization_native.py
python3 -m pytest -q tools/test_editor_localization.py
python3 -m pytest -q magnolie-organizer/pruefungen/test_locale_completeness.py magnolie-organizer/pruefungen/test_pot_source_coverage.py magnolie-organizer-windows/tests/test_windows_pot_source_coverage.py
MAGNOLIE_PYTHON=/usr/bin/python3 /home/maik3531/.local/bin/deno run --unstable-detect-cjs --cached-only --allow-read --allow-write=/tmp --allow-env --allow-run=/usr/bin/python3 magnolie-organizer-windows/tests/localization-completeness.js
```
