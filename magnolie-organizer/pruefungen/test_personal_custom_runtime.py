...
def test_desktop_actual_snapshot_functions_validate_on_python():
    ...
    result = subprocess.run([runtime, str(ROOT.parent / "magnolie-organizer-windows/tests/personal-custom-snapshot.js"), "--emit"],
...
def test_all_twenty_manual_resources_match_frozen_text():
    ...
    native = json.loads((ROOT.parent / "magnolie-organizer-windows/app/native-i18n.json").read_text())["locales"]
    for locale, values in data["translations"].items():
        folder = "values" if locale == "en" else "values-" + ("zh-rCN" if locale == "zh_CN" else locale)
        xml = ET.parse(ROOT.parent / "magnolie-notes/app/src/main/res" / folder / "strings.xml")
        ...
        for index, (key, msgid, text) in enumerate(zip(data["ids"], data["translations"]["en"], values)):
            ...
            if locale != "en" and index in (0, 2, 3):
                ...
                for base in (ROOT / "po", ROOT.parent / "magnolie-organizer-windows/app/po"):
