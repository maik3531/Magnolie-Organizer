...
def test_android_direction_and_native_control_source_boundaries():
    root = BIN.parents[1]
    android = root / "magnolie-notes/app/src/main/java/io/gitlab/maik3531/magnolienotes/telefon"
...
def test_manual_localizations_have_all_twenty_languages_and_unique_keys():
    ...
    native = json.loads((BIN.parents[1] / "magnolie-organizer-windows/app/native-i18n.json").read_text())
    for locale, values in module.TRANSLATIONS.items():
        ...
        if locale != "en":
            ...
            for directory in (BIN.parent / "po", BIN.parents[1] / "magnolie-organizer-windows/app/po"):
