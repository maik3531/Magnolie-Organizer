...
object PortableArchiv {
    ...
    fun erstellen(bestand: Bestand, passwort: CharArray): ByteArray {
        ...
        val portable = portable(bestand)
        val klar = Ablage.json.encodeToString(PortableNutzlast.serializer(), PortableNutzlast(
            createdUtc = Instant.now().toString(), bestand = portable)).toByteArray(Charsets.UTF_8)
        ...
        val kopf = kopf(salz, nonce, klar.size + TAG_BYTES)
    ...
    fun pruefen(archiv: ByteArray, passwort: CharArray): GeprueftesPortableArchiv {
        ...
        return try {
            ...
            require(payload.type == "magnolie-portable-bestand" && payload.version == 1)
            Instant.parse(payload.createdUtc)
            val bestand = portable(payload.bestand)
            require(bestand == payload.bestand) { "Nicht portabler Bestand" }
    ...
    private fun portable(bestand: Bestand): Bestand = bestand.copy(
        notizen = bestand.notizen.map(::portable),
        aufgaben = bestand.aufgaben.map(::portable),
    ...
    private fun kopf(salz: ByteArray, nonce: ByteArray, geheimLaenge: Int): ByteArray {
        val aus = ByteArrayOutputStream()
        DataOutputStream(aus).use {
            it.write(MAGIC); it.writeByte(1); it.writeByte(1); it.writeByte(1)
